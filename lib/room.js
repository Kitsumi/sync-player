...
class Room {
    ...
    onUserMessage(user, message) {
        if (message.type == "request_video") {
            if (Date.now() - user.lastQueue < 5000) return;
            user.lastQueue = Date.now();
        ...
        } else if (message.type == "change_name" && typeof message.name == "string" && message.name.length < 32) {
            if (Date.now() - user.lastNameChange < 500) return;
            user.lastNameChange = Date.now();

            let t = message.name.replace(/[^a-z0-9 ]/gi, "").replace(/\s+/gi, " ").trim();
            if (t.length >= 2) {
            ...
            if (Date.now() - user.lastVoteSkip < 500) return;
            user.lastVoteSkip = Date.now();
